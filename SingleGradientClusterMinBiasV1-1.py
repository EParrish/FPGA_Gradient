...
def GetGTowers(eventNum,datafile_path,treeName):
    ...
    print "This is the range ", len(es)
    for i in range(ngTowers):
        gTowers.append(TLorentzVector(0,0,0,0))
        gTowers[i].SetPtEtaPhiM(ets[i],etas[i],phis[i],0)
        #print ets[i] , gTowers[i].Et(), " also ", etas[i] , gTowers[i].Eta(), " also ", phis[i], gTowers[i].Phi()
...
def plotAllArrays(arrayOfgTowers,gArray,eArray,fArray,eventNum,GradThreshold):
    ...
    H1 = TH2F("Event Display","MinBias Event %s Display: E_{T};#eta;#phi" %(eventNum),32,-3.2,3.2,32,-3.2,3.2)
    H2 = TH2F("Event Display","MinBias Event %s Display: #Delta E_{T};#eta;#phi" %(eventNum),32,-3.2,3.2,32,-3.2,3.2)
    H3 = TH2F("Event Display","MinBias Event %s: %s #Delta E_{T} Cut;#eta;#phi" %(eventNum,GradThreshold),32,-3.2,3.2,32,-3.2,3.2)
    H4 = TH2F("Event Display","MinBias Event %s: Final E_{T} (%s#Delta E_{T} Cut);#eta;#phi" %(eventNum,GradThreshold),32,-3.2,3.2,32,-3.2,3.2)
    H5 = TProfile("Profile Display","MinBias Event %s: E_{T} Profile" %(eventNum),32,-3.2,3.2)
    H6 = TH1F("ET Distribution","MinBias Event %s: E_{T};E_{T} [GeV];Number of gTowers / 0.5 GeV" %(eventNum), 200,-50,50)
    H7 = TH1F("Gradient Distribution","MinBias Event %s: #DeltaE_{T};#DeltaE_{T} [#DeltaGeV];Number of gTowers / 0.5 #DeltaGeV"%(eventNum), 200,-50,50)
    ...
    if DisplayBool_ == 'y':
        ...
        H1.SaveAs("/export/home/llisi/SingleEventInfo/MinBiasFullEventDisplay-Event%s.root" %(eventNum))
        H2.SaveAs("/export/home/llisi/SingleEventInfo/MinBiasFullGradientDisplay-Event%s.root" %(eventNum))
        H3.SaveAs("/export/home/llisi/SingleEventInfo/MinBiasCutGradientDisplay-Event%s-GThresh%s.root" %(eventNum,GradThreshold))
        H4.SaveAs("/export/home/llisi/SingleEventInfo/MinBiasPostCutEtDisplay-Event%s-GThresh%s.root" %(eventNum,GradThreshold))
        H5.SaveAs("/export/home/llisi/SingleEventInfo/MinBiasProfileDisplay-Event%s.root" %(eventNum))
        H6.SaveAs("/export/home/llisi/SingleEventInfo/MinBias1dE_TDistribution-Event%s-GThresh%s.root" %(eventNum,GradThreshold))
        H7.SaveAs("/export/home/llisi/SingleEventInfo/MinBias1dDeltaE_TDistribution-Event%s-GThresh%s.root" %(eventNum,GradThreshold))
    
    else:
        raise Exception('Please enter y or n for whether you would like to display.')
...
DisplayBool_ = raw_input('Would you like to display the plots? y/n')


MakeNewFile('/export/home/llisi/SingleEventInfo/MinBiasEventInfoDE%sGeV-Event%s.root' %(gThresh_,EventNum_), '/headprv/atlas/local/eparrish/Samples/V11/MinBias/TEMP/PileupSkim_nu_E50-V11-0_TEMP.root','mytree',gThresh_,NeighborBool_,EventNum_)
